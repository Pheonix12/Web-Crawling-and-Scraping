...
PATH_KEYWORD = "/example/path"
USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3"

# Base directory for saving files
BASE_DIRECTORY = Path('text_1').joinpath(DOMAIN.replace('.', '_'))
BASE_DIRECTORY.mkdir(parents=True, exist_ok=True)

# State directories
STATE_DIR = Path('text_1/crawl_state')
...
def get_unique_filename(base_path):
    ...
    while base_path.exists():
        base_path = base_path.with_name(f"{original_stem}_{counter}{base_path.suffix}")
        counter += 1
    return base_path
...
def sanitize_filename(url):
    ...
    file_path = BASE_DIRECTORY / (url_hash + '.txt')
...
if __name__ == "__main__":
    ...
    initial_urls = ["https://www.example.com/path"]
    asyncio.run(parallel_crawl(initial_urls, concurrency=4))
